...
const EventDetails = () => {
  ...
  // Track page view quando evento carregar
  useEffect(() => {
    if (event) {
      trackPageView(`/event/${id}`, id);
    }
  }, [event, id, trackPageView]);

  // Handler para mudar de tab
  const handleTabChange = (tab: string) => {
    setActiveTab(tab as any);
    if (event) {
      trackClick(event.id, tab);
    }
  };

  // Atualizar quantidade de ingressos
  const updateQuantity = (tierId: string, delta: number) => {
    const newQty = Math.max(0, (quantities[tierId] || 0) + delta);
  ...
  const handleCheckout = () => {
    ...
    // Navegar apenas se houver itens no carrinho
    if (cart.length > 0) {
      trackClick(event.id, 'checkout');
      navigate('/checkout', { 
        state: { 
          cart, 
          eventTitle: event.title,
          eventId: event.id // Corrigido: eventId (minúsculo)
        } 
      });
    }
  };
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            <div className="bg-zinc-50 p-6 rounded-lg border">
              ...
              <div className="aspect-video bg-zinc-200 rounded-lg mt-6 overflow-hidden relative">
                ...
                <img src={`https://picsum.photos/seed/location${id}/1000/600`} alt="Map" className="w-full h-full object-cover opacity-60" />
              </div>
            </div>
          </div>
        )}
